...
class AdvancedCoverLetterService
  ...
  def analyze_cover_letter_sequential(content, user_name = nil)
    # YAML 파일에서 프롬프트 로드 시도
    prompt_data = get_prompt(:full_analysis, { user_name: user_name || "지원자", content: content })
    
    if prompt_data && prompt_data[:prompt]
      # YAML 파일의 프롬프트 사용
      prompt = prompt_data[:prompt]
      max_tokens = prompt_data[:max_tokens] || 8000
    else
      Rails.logger.warn "YAML 프롬프트를 찾을 수 없어 기본 프롬프트 사용"
      # Fallback으로 기존 프롬프트 사용 (임시)
      prompt = <<~PROMPT
      ✅ 최종 자기소개서 분석용 프롬프트
      ...
      **반드시 4-5문단, 각 문단 5-7문장으로 작성**
      ...
      전체적인 첫인상과 구조를 평가해주세요.
      논리성과 일관성을 구체적 인용과 함께 평가해주세요.
      진정성과 차별성을 분석해주세요.
      직무 적합성과 기업 이해도를 평가해주세요.
      종합적인 평가와 개선 방향을 제시해주세요.
      ...
      **절대 사용 금지 표현:**
      ...
      자소서 내용을 인용하며 첫인상을 설명해주세요. (5-7문장으로 상세히)
      ...
      발전 가능성과 조직 기여도를 평가해주세요. (5-7문장으로 상세히)
      ...
      왜 이것이 문제가 되는지 실제 사례와 함께 설명해주세요.
