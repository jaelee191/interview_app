...
class AdvancedCoverLetterService
  ...
  # 피드백 기반 개선된 자기소개서 생성
  def generate_improved_letter(original_content, feedback_analysis, company_name = nil, position = nil)
    prompt = <<~PROMPT
      ...
      ⭐️ 중요한 출력 형식 규칙:
      각 항목(1. 지원동기, 2. 성장과정, 3. 협업경험 등)은 반드시 다음 형식으로 작성하세요:
      ...
      **형식 예시:**
      ...
      👉 작성 규칙:
      1. 각 항목마다 반드시 [한 줄 제목] + 3개 문단 구성을 지켜주세요.
      2. 한 줄 제목은 해당 항목의 핵심 메시지를 압축적으로 표현합니다.
      3. 각 문단은 최소 4-5문장 이상으로 충실하게 작성합니다.
      4. 모든 경험은 **상황 → 행동 → 성과 → 배운 점 → 직무 연결**의 구조로 작성합니다.
      5. 피드백에서 제시된 모든 개선점을 반드시 수정하여 반영합니다:
      ...
      원본의 모든 항목(1. 지원동기, 2. 성장과정, 3. 협업경험, 4. 도전경험, 5. 직무역량, 6. 장점 및 단점 등)을 빠짐없이 포함하되,
      각 항목마다 [한 줄 제목] + 3개 문단 형식으로 완전히 새롭게 작성합니다.
      
      ⚠️ 중요: 
      - 원본에 있는 모든 항목을 빠짐없이 포함하고, 절대 중간에 끊기지 않도록 끝까지 완성해주세요.
      - 각 항목마다 [한 줄 제목] + 3개 문단 구성을 반드시 지켜주세요.
      - 번호와 항목명은 원본과 동일하게 유지하세요 (예: 1. 지원동기, 2. 성장과정...)
    PROMPT
