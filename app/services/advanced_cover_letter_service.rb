...
class AdvancedCoverLetterService
  ...
  def generate_improved_letter(original_content, feedback_analysis, company_name = nil, position = nil)
    prompt = <<~PROMPT
      ...
      👉 작성 규칙:
      1. 각 문항(지원동기/직무역량/장단점 등)에 맞게 문단을 재구성합니다.
      2. 모든 경험은 반드시 **상황 → 행동 → 성과 → 배운 점 → 직무 연결**의 구조로 작성합니다.
      3. 피드백에서 제시된 모든 개선점을 반드시 수정하여 반영합니다:
      ...
      4. 지나치게 짧은 표현이나 단순 나열식 금지. 각 문항은 최소 3-4문단 이상으로 확장합니다.
      5. 강점은 구체적 사례와 수치로 강조하고, 단점은 실제 실패 경험과 개선 과정을 포함합니다.
      6. 모든 추상적 표현을 구체적 에피소드로 대체합니다.
      7. STAR 기법(Situation-Task-Action-Result)을 철저히 적용합니다.
      8. 글 전체의 톤은 진정성 있고 설득력 있게, HR 담당자가 읽기 편한 문체로 작성합니다.
      9. **격려 멘토 코멘트는 포함하지 않고, 최종 자기소개서 원고 형태로만 출력**합니다.
